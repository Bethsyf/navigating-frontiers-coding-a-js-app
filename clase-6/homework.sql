...
SELECT g.id, g.name, g.color, u.name AS userName
FROM Groups g
INNER JOIN Users u ON g.ownerUserId = u.id
WHERE u.name = 'juan camilo';
...
FROM Groups g
INNER JOIN Users u ON g.ownerUserId = u.id
WHERE u.name IN ('miguel', 'liliana');
...
SELECT * 
FROM Groups ORDER BY name DESC;

-- select only the first 5 groups ordered by name asc 

SELECT * 
FROM Groups ORDER BY name ASC LIMIT 5;
...
SELECT g.id, g.name, g.color, g.createdAt, u.name AS userName
FROM Groups g
INNER JOIN Users u ON g.ownerUserId = u.id;
